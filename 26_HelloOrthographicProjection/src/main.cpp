...
void prepareOrthographicMatrix() {
    orthographicMatrix = glm::ortho(-1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f);
}
void preparePerspectiveMatrix() {
    ...
    // zNear: 近平面的距离
    // zFar: 远平面的距离
    perspectiveMatrix = glm::perspective(glm::radians(45.0f), (float)(lwapp->getWidth() / lwapp->getHeight()), 0.1f, 1000.0f);
}
